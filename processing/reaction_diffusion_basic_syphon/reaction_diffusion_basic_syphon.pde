...
void setup(){
  ...
  // Seed 10 cells
  for (int n = 0; n < 10; n++) {
    // Choosing 10 random spots within screen to seed
 ...
 for (int i = 1; i < width - 1; i ++) {
   for (int j = 1; j < height - 1; j ++) {
     ...
     float laplaceA = 0;
      ...
      newspot.a = a + (dA*laplaceA - a*b*b + feed*(1 - a))*1.1;
      newspot.b = b + (dB*laplaceB + a*b*b - (k+feed)*b)*1.1;
...
void draw(){
  ...
  // Why need this? only update once per cycle?
  for(int i = 0; i < 1; i++){
    update();
    swap();
  }
  ...
  for (int i = 1; i < width - 1; i ++){
    for (int j = 1; j < height -1; j ++) {
      ...
      float b = spot.b;
      int pos = i + j * width;
      canvas.pixels[pos] = color((a-b)*255);
    }
  }
  canvas.updatePixels();
  canvas.endDraw();
